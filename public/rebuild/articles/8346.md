...
<<>>发现一个比较轻量的CI/CD工具：#[Drone](https://drone.io)，成功给本站安排上了，但是只安排了一点![sticker](aru/104)（下面会说明），地址是#[https:\//blog-cf.yunyuyuan.net/](https://blog-cf.yunyuyuan.net/)，运行在我家里的linux服务器上，使用cloudflared tunnel做内网穿透，关于cloudflared tunnel详见#[之前的文章](https://blog.yunyuyuan.net/articles/5896)。
...
### 新增环境变量与cronjob
<<>>新增两个环境变量，`NB_PASSWD`和`NB_IMG_REGEX`，分别代表密码和正则表达式（新增可选变量：`FILE_USER`和`FILE_GROUP`，分别代表下载的图片的owner id和group id）。新增一个名为`backup_img`的cronjob，每周执行：
