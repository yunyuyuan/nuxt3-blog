...
<<>>做一个#[门户界面](https://github.com/yunyuyuan/self-host-starter/blob/main/interface.html)，把服务的URL都放到上面，切换是否使用代理，显示不同的链接。不代理的url是`http://file-direct.example.com:9527`，代理的url是`http://file.example.com`：
